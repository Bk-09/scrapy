...
class CrawlSpider(Spider):
    ...
    def parse_start_url(self, response: Response, **kwargs: Any) -> Any:
        ...
        containers = response.xpath('//div[@class="ds-border-b ds-border-line ds-p-4"]')
    
        for container in containers:
            title = container.xpath('.//h2/text()').get()
            subtitle = container.xpath('.//p/text()').get()
            link = container.xpath('.//a/@href').get()
            articles.append({'title': title, 'subtitle': subtitle, 'link': link})

        #Apply Filter
        keyword = 'cricket'  # Adjust the keyword as needed
        filtered_articles = filter_articles_by_keyword(articles, keyword)

        # Convert filtered articles to DataFrame
        import pandas as pd
        df_filtered_headlines = pd.DataFrame(filtered_articles)
        df_filtered_headlines.to_csv('filtered_headlines.csv')
